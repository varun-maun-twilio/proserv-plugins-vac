...
import { canRecordTask, recordExternalCall, recordInternalCall } from '../../helpers/outboundRecordingHelper';
...
export const eventHook = async (flex: typeof Flex, _manager: Flex.Manager, task: Flex.ITask) => {
  ...
  if (!canRecordTask(task)) {
    logger.info(`[outbound-recording] Skipping recording for task excluded by configuration: ${task.sid}`);
    return;
  }
  ...
  if (client_call && direction === 'outbound') {
    ...
    logger.info(`[outbound-recording] Skipping recording for inbound internal call ${task.sid}`);
  } else {
    // External call
    // Do not await so that event processing is not blocked
